		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($order_detail_list as $key => $order_detail)
			{
				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					$payment_type = substr_count($order->payment, 'Яндекс.Касса') ? 'Яндекс.Касса' : $order->payment;
					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
					{
						...
						// Join PDF invoice
						if ((int)Configuration::get('PS_INVOICE') && $order_status->invoice && $order->payment == "Банковский перевод")
						{
//							$pdf = new PDF($order->getInvoicesCollection(), PDF::TEMPLATE_INVOICE, $this->context->smarty);
//							$file_attachement['content'] = $pdf->render(false);
							require_once _PS_MODULE_DIR_.'bankformcom/bankformcom.php';
//							 $file_attachement['content'] = BankFormCom::generatePDF($order->id); //Рендер накладной через модуль
//							 $file_attachement['name'] = Configuration::get('PS_INVOICE_PREFIX', (int)$order->id_lang, null, $order->id_shop).sprintf('%06d', $order->invoice_number).'.pdf';
//							 $file_attachement['mime'] = 'application/pdf';
							$file_attachement['content'] = BankFormCom::generateDOC($order->id); //Рендер накладной через модуль
							$file_attachement['name'] ='Заказ #'.$order->reference.'.doc';
							$file_attachement['mime'] = 'application/msword';
						}
						else
