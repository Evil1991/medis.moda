...
class Hforms
{
    ...
    public function getFormYamoneyPaySettings()
    {
        $states = Db::getInstance()->executeS('SELECT id_order_state as id, `name` FROM ' . _DB_PREFIX_ . 'order_state_lang WHERE id_lang = 1 ORDER BY id ');

        return array(
            'form' => array(
                'legend' => array(
                    'title' => $this->l('Настройки отложеной покупки'),
                    'icon' => 'icon-cogs',
                ),
                'input' => array(
                    array(
                        ...
                        'class' => 't',
                        'type' => 'text',
                        'desc' => $this->l('Текст после подтверждения заказа'),
                        'name' => 'YA_LATE_CONFIRMATION_TEXT',
                        'label' => $this->l('Текст после подтверждения заказа'),
                    ),
                    array(
                        'type' => 'select',
                        'label' => $this->l('Статус заказа после оформления'),
                        'name' => 'YA_LATE_PRE_STATUS',
                        'desc' =>$this->l('Статус заказ, после нажатия кнопки купить'),
                        'options' => array(
                            'query' => $states,
                            'name' => 'name',
                            'id' => 'id'
                    ...
                    array(
                        'type' => 'select',
                        'label' => $this->l('Статус заказа для покупки'),
                        'name' => 'YA_LATE_ORDER_STATUS',
                        'desc' =>$this->l('Статус заказа, при котором он становится для покупки'),
                        'options' => array(
                            'query' => $states,
                            'name' => 'name',
                            'id' => 'id'
