...
class Yamodule extends PaymentModule
{
    ...
    public function hookActionOrderStatusPostUpdate($params)
    {
        ...
        if($status->id == Configuration::get('YA_LATE_ORDER_STATUS')) {
            ...
            $params = array(
                ...
                '{orderUrl}' => $link->getModuleLink('yamodule', 'paymentorder', array('id_order' => $order->id)),
            );

            Mail::Send(
                (int)$order->id_lang,
                'payment_ready',
                sprintf(Mail::l('Вы можете оплатить заказ %s', (int)$order->id_lang), $order->reference),
                $params,
                $customer->email,
    ...
    public function selfPostProcess()
    {
        ...
        } elseif (Tools::isSubmit('submitYaLateModule')) {
            $this->org_status = $this->validateYaLate();
            $this->update_status = $this->sendStatistics();//$this->sendStatistics();
    ...
    public function validateYaLate()
    {
        Configuration::UpdateValue('YA_LATE_CONFIRMATION_TEXT', Tools::getValue('YA_LATE_CONFIRMATION_TEXT'));
        Configuration::UpdateValue('YA_LATE_PRE_STATUS', Tools::getValue('YA_LATE_PRE_STATUS'));
        Configuration::UpdateValue('YA_LATE_ORDER_STATUS', Tools::getValue('YA_LATE_ORDER_STATUS'));
    }
    ...
    public function getContent()
    {
        ...
        $vars_ya_late = Configuration::getMultiple(array(
            'YA_LATE_CONFIRMATION_TEXT',
            'YA_LATE_PRE_STATUS',
            'YA_LATE_ORDER_STATUS',
        ));
        ...
        $this->context->smarty->assign(array(
            ...
            'money_late'    => $this->renderForm('YaLate', $vars_ya_late, $hforms->getFormYamoneyPaySettings())
        ));
