	...
	<div class="primary_block row">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-5">
			...
			<div id="image-block" class="clearfix">
				{if $product->new}
					<span class="new-box">
						<span class="new-label"><span class="new_span">{l s='New'}</span></span>
					</span>
				{/if}
				{if $product->on_sale}
					<span class="sale-box no-print">
						<span class="sale-label"><span class="new_span">{l s='Sale!'}</span></span>
					</span>
				...
				{if $have_image}
					<span id="view_full_size">
						{if $jqZoomEnabled && $have_image && !$content_only}
							<a class="jqzoom" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" rel="gal1" href="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'watermark')|escape:'html':'UTF-8'}" itemprop="url">
								<img itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html':'UTF-8'}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}"/>
							</a>
						{else}
							<img id="bigpic" itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" width="{$largeSize.width}" height="{$largeSize.height}"/>
							{if !$content_only}
						...
						{if isset($images)}
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'watermark')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'watermark')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			{if !$content_only}
				...
				{*soopa table by prestaweb*}
				{if isset($groups.1) && isset($groups.2)}
					<div id="attributeTable">
						<div id="matrix">
							<p class="matrix-title">Выберите размер и рост</p>
							<p class="sizetable matrix-title" style="cursor: pointer; text-decoration: underline; color: #00B06C">Определить свой размер</p>
							<script type="text/javascript">
								$('p.sizetable').on('click', function(){
									$.fancybox({
										href: '/content/15-opredelite-svoj-razmer #table',
										type: 'ajax'
									});
								});
							</script>
							<table id="matrix_table">
								<thead>
								<th class="topRow"></th>
								{foreach from=$groups.1.attributes key=k item=att}
									<th data-id="{$k}">{$att}</th>
								{/foreach}
								</thead>
								<tbody>
								{foreach from=$groups.2.attributes key=k item=att}
									<tr data-id="{$k}">
										<td class="leftCol">{$att}</td>
										{section name=loop loop=count($groups.1.attributes)}
											<td><a href="#" class="notsel">&nbsp;</a></td>
										{/section}
									</tr>
								{/foreach}
								</tbody>
				...
				<div class="box-info-product">
							...
							<div id="attributes">
								...
								{foreach from=$groups key=id_attribute_group item=group}
									{if $group.attributes|@count}
										<fieldset class="attribute_fieldset {if isset($groups.1) && isset($groups.2) && ($id_attribute_group==1 || $id_attribute_group==2)}hidden{/if}">
					...
					<div class="box-cart-bottom">
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							...
							{*<span class="cpok" style="display:none;">Срок изготовления <b>7</b> дней</span>*}
							<span class="cpok">{$product->upc|escape:'html':'UTF-8'}</span>
						</div>
