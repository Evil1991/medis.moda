		...
		<table id="order-list" class="table table-bordered footab">
			<thead>
				<tr>
					...
					<th data-breakpoints="cs" data-hide="phone" class="item">{l s='Date'}</th>
					<th data-breakpoints="phone" data-hide="phone" class="item">{l s='Total price'}</th>
					<th data-breakpoints="phone" data-sort-ignore="true" data-hide="phone,tablet" class="item">{l s='Payment'}</th>
					<th data-breakpoints="phone" data-hide="phone, tablet" class="item">{l s='Status'}</th>
					...
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						<td{if isset($order.order_state)} data-value="{$order.id_order_state}"{/if} class="history_state">
							{if isset($order.order_state)}
                                {if $order.id_order_state == Configuration::get('PS_OS_PAYMENT') || $order.id_order_state == 12}
									<span class="label{if isset($order.order_state_color) && Tools::getBrightness($order.order_state_color) > 128} dark{/if}" style="background-color:Crimson; border-color:Crimson;">
										Оплачен
									</span>
                                {else}
						...
						<td class="history_invoice">
							{if $order.id_order_state == Configuration::get('YA_LATE_ORDER_STATUS')}
								<a href="/module/yamodule/paymentorder?id_order={$order.id_order}" class="label" style="background-color:red; border-color:red;">Оплатить</a>
							{/if}
							{if $order.id_order_state == Configuration::get('PS_OS_PAYMENT') || $order.id_order_state == 12}
								<span class="label{if isset($order.order_state_color) && Tools::getBrightness($order.order_state_color) > 128} dark{/if}"{if isset($order.order_state_color) && $order.order_state_color} style="background-color:{$order.order_state_color|escape:'html':'UTF-8'}; border-color:{$order.order_state_color|escape:'html':'UTF-8'};"{/if}>
									Оплачен
